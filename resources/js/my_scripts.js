...
function viewStudentStats(id, toggle) {
	var htmlTagId = document.getElementById(id);
	switch(toggle) {
		case 0: // handles when the toggle is 0
		  htmlTagId.style.visibility = "hidden" // sets the visibility to hidden
		  htmlTagId.style.height = 0
		  break;
		case 1: // handfles when the toggle is 1
		 htmlTagId.style.visibility = "visible" // sets the visibility to visible
		 htmlTagId.style.height = auto 
		  break;
		default:
		  // code block
	  }
...
// HomePage 
	function changeColor(color) {
		document.body.style.backgroundColor = color // changing the color of the background.
	}
...
function loadPlayersPage() {
	...
	players.map(function(item) { return '<a href="#" onclick="switchPlayers()">' + item + '</a>' });
	for (i = 0; i < players.length; i++) {
		player = players[i];
		console.log(player.name);

		if (i < 3 ) {
			nextPlayerName = players[i+1].name;
		} else {
...
function switchPlayers(playerNum) {
	...
	avg_p_yards.innerHTML = parseInt(p_yards / player.games_played);
	avg_r_yards.innerHTML = parseInt(r_yards / player.games_played);
	avg_rec_yards.innerHTML = parseInt(rec_yards / player.games_played);
